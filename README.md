...
* **宏觀規劃角色 (Strategic Planner / 規劃大腦)**：負責分析原始目標、拆解 task queue 並制定 Roadmap。可由雲端大模型、本機大模型或專用規劃 Prompt 擔任。
* **微觀執行角色 (Tactical Executor / 執行手腳)**：領取單一任務卷宗，在受控環境（本機或沙箱）內編修產出、進行事實核對與驗證。可由輕量模型、本機開源模型或專用執行 Prompt 擔任。

> [!TIP]
> **💡 單一模型全流程相容**：如果您想完全離線、保護隱私或節省成本，可使用**本機單一 27B/32B/70B 等模型**同時擔任「規劃大腦」與「執行手腳」跑完全部流程，絕無強制要求雲地混合。
...
3. **自動化檢驗與人類極簡驗收 (AI Self-Review & Minimal Human Handoff)**：
   * **AI 自動復盤與 3 次自癒限制**：AI 在回報人類前，會先自動對照 `recipe.md` 的 DoD 進行自我檢驗與本地測試。若有任何問題，AI 會在後台默默修復（最大連續限制 3 次以防止死循環與 token 浪費），3 次自癒失敗將自動掛起為 `ESCALATED` 狀態。
...
* 🧠 **[Tier 1: 核心大腦 (Local-Agent-Workspace)](https://github.com/Chiakai-Chang/Local-Agent-Workspace)**：
  ...
  * **作用**：承接宏觀規劃角色的規劃，作為進行打底運算的強大本地/私有算力引擎。
  * 📍 **您目前在這裡**
* 🤖 **[Tier 2: 代理工程師 (CK's Pi Code Agent Harness)](https://github.com/Chiakai-Chang/CKs_PI_Code_Agent_Harness)**：
  * **定位**：微觀開發與執行的指揮樞紐。
  * **任務**：接收規劃角色開出的「任務卷宗與指引（Recipe）」，在執行端化身為嚴守工程紀律的虛擬同事。
  * **執行**：按部就班地切換目標檔案、撰寫程式碼，並在執行端嚴格執行 TDD 單元測試。
* 👁️ **[Tier 3: 全域修復雷達 (OmniHeal)](https://github.com/Chiakai-Chang/OmniHeal)**：
  * **定位**：零安裝的全局專案健檢與自動修復診斷器。
  * **任務**：一鍵免費深潛分析，自動抓出專案中的技術債，並開立精準的修復處方箋。
  * **優勢**：解決全案掃描中最耗費 Token 的環節，並引導規劃角色或執行角色進行針對性修復。
