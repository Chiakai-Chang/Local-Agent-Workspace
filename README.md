...
* **`-c 16384`**：預設限制在 16K。**CPU 運行的關鍵優勢在於系統主記憶體 (RAM) 容量充足 (如 64GB)**，這能讓您輕鬆載入高精度模型版本 (如 `IQ4_XS` 甚至更高等級) 且能把 `-c` 上下文開得非常大 (如 32K 或 64K) 而不用擔心 GPU VRAM 溢位 (OOM)。
* **💡 記憶體容量與 Prefill 效能權衡 (Trade-off)**：雖然大 RAM 帶來了高精度與超長 Context 的優勢，但**由於 CPU 的記憶體頻寬遠低於 GPU，這會導致 Prefill (提示詞預評估) 階段的速度非常緩慢**，大上下文時首字生成時間 (TTFT) 會拉得比較長。若您的應用場景著重於大上下文處理，且不介意首字延遲，可視系統 RAM 大小自由調大此數值。
