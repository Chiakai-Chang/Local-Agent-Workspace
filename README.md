...
## 💎 部署本地環境的優勢
  ...
  * **🧠 高上下文容量：** 透過 8-bit KV Cache 快取壓縮技術，在 20GB VRAM 的環境下依然可支援至 **64K~128K Context**，足以應付大規模 Repo 分析。
...
我們目前推薦使用更新最快、社群支援度最高的官方 **Llama.cpp** 作為核心引擎。

1. 請至 [Llama.cpp 最新 Release](https://github.com/ggml-org/llama.cpp/releases) 頁面。
2. 下載適用於 Windows x64 的版本（如 `cudart-llama-bin-win-cu12.4-x64.zip`）。
3. 建立專屬資料夾（請自訂路徑，例如：`C:\llama.cpp`），並將解壓縮後的檔案放入其中。

*(註：目前官方版 Llama.cpp 搭配 CUDA 12.4 DLLs 在效能與穩定度上已極度強大，足以應付絕大多數的高強度開發場景。)*
...
請將以下代碼存成 `start_server.bat`，並與您的 `llama-server.exe` 放在同一目錄下執行。

此腳本已針對 RTX A4500 (20GB VRAM) 進行深度最佳化，無論您選擇上述哪一款模型皆適用。

> [!CAUTION]
> **執行前請務必修改路徑：**
> 請將腳本中的 `<您的_llama.cpp_資料夾路徑>` 與 `<您的模型存放路徑>` 替換為您電腦中實際的位置與檔名！
