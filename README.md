...
軌道二專注於 AI Agent 的工程紀律管束。此處設計可完美兼容以下多種部署拓撲，支援彈性配比（全本地 Full-Local、全雲端 Full-Cloud、或雲地混合 Hybrid）：
* **宏觀規劃角色 (Strategic Planner / 規劃大腦)**：負責分析任務、拆解 task queue 並制定 Roadmap。可由雲端大模型、本機大模型或專用規劃 Prompt 擔任。
* **微觀執行角色 (Tactical Executor / 執行手腳)**：領取單一任務卷宗，在受控環境（本機或沙箱）內撰寫代碼與執行測試。可由輕量模型、本機開源模型或專用執行 Prompt 擔任。
